...
CURR_GRADE = 0
PREV_GRADE = 1
ACQ_REPS = 9 # NO_IMPACT (was 2)
EASINESS = 2
RET_REPS = 3
LAPSES = 4
ACQ_REPS_SINCE_LAPSE = 10 #(was 5)
ACTUAL_INTERVAL = 5
RET_REPS_SINCE_LAPSE = 6
THINKING_TIME = 7
THINKING_TIME_PREV = 8


def get_training_data(limit=1000):
    users = mnemosyne_logs.list_user_ids(limit)
    num_examples = 0
    training_x = np.ones((limit,11), dtype=float)
    ...
    for user in users:
        ...
        for card in logs:
            ...
            for i in range(1, num_reps):
                ...
                training_x[num_examples][THINKING_TIME] = curr[THINKING_TIME]
                training_x[num_examples][THINKING_TIME_PREV] = prev[THINKING_TIME]
