"""
mnemosyne_logs.py
Interface with the raw Mnemosyne logs database.
...
def fetch_logs(user_id):
    """Returns a list of lists, where each sublist contains the repetition logs
    for a single card used by the given user, ordered chronologically from earliest
    to latest.
    """
    c = conn.cursor()
    ...
    for object_id in object_ids:
        c.execute('SELECT * FROM log WHERE user_id = "%s" AND object_id = "%s" AND %s ORDER BY timestamp ASC'
